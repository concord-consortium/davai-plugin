...
export const createGraphTool: ILocalTool = {
  ...
  validate(args, ctx) {
    ...
    // Default here, at validate time (x/y are already resolved), so the default is always sent
    // to CODAP, not just used for display. Empty-string title counts as absent, same as
  ...
  async execute(resolved, ctx) {
    ...
    // An empty-string echo from CODAP counts as absent (guard against `res?.values?.title` being
    // a real API value of "") — falls back to our own (already-sent) default rather than
    // printing a blank.
