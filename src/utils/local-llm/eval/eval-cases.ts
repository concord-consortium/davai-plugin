...
export const evalCases: IEvalCase[] = [
  // A redundant-but-harmless fetch (the seed already has the data) uses allowOnly rather than a
  // strict `none`, so a model isn't failed for double-checking. Grounding is strengthened to a
  // specific value from the Mammals/Height fixture, not just the word "height".
  ...
  // The prompt explicitly asks "how many you selected" so the answer grounds a digit, matching
  // the regex below.
  ...
  // Exercises resolveGraph's rung 3 (axis + plot-type matching). create-graph (above) makes a
  // graph titled "Height vs Mass" — this prompt flips the word order to "Mass vs Height", so
  // title normalization alone cannot match it; only axis-substring scoring can. Case ORDER
  // matters here: this must run after create-graph creates the graph and before any case that
  // would create another graph sharing both axis names.
  // The /\d/ requirement reflects that get_graph_info appends a client-computed sketch (range,
  ...
  // Height times 3.281 is the correct meters-to-feet conversion factor (not /30.48, the
  // cm-to-feet divisor).
  ...
  // Exercises update_attribute's unit field on an existing (non-formula) attribute — a distinct
  // capability from create-attribute-formula immediately above (which exercises create_attribute's
  // formula field on a NEW attribute).
  ...
  // "The Height graph" is named explicitly because "the graph" is ambiguous once create-graph
  // (above) has run — the sonification store auto-selects the newest graph, so an unqualified
  // "the graph" would resolve to the Height-vs-Mass scatterplot, where Mean is legitimately
  // unavailable.
  ...
  // Exercises update_graph's rung-1 exact-title graph resolution and its y-axis change path.
  // Runs after create-adornment (which needs the pre-change Height graph still intact) and before
  // group-by-diet.
  ...
  // Exercises group_by's happy path (new parent collection + attribute move) and grounded
  ...
  { id: "miscapitalized-attribute", prompt: "what is the mean of hieght?",   // misspelled + lowercase on purpose
    expectTools: { contains: ["get_stats"] }, expectFinal: { matches: [/mean/i, /\d/] } },
  // Exercises find_cases's orderBy-form grounded lookup (the a11y priority) — a natural-language
  // superlative question, no explicit tool/attribute name in the prompt.
  ...
  // A Diet x Habitat graph (both categorical) is created here so the next case can describe it.
  // Runs after find-heaviest (which relies on the post-group_by hierarchy from group-by-diet);
  // creating a second graph here doesn't disturb that prior state since find-heaviest has already
  // run.
  { id: "create-categorical-graph", prompt: "Make a graph of Diet versus Habitat.",
    expectTools: { contains: ["create_graph"] }, expectFinal: { matches: [/graph|plot/i], notMatches: [/fail/i] } },
  // Regression guard against the categorical-graph hallucination: without a real sketch,
  // get_graph_info would hand the model structure only for a categorical x categorical graph, and
  // the model fills that vacuum with invented world-knowledge categories (e.g. "herbivore,
  // carnivore, omnivore", "forest, grassland, aquatic") that don't exist anywhere in the Mammals
  // dataset. The notMatches list below is EXACTLY that hallucination.
