import {ClientHandler, codapInterface} from "@concord-consortium/codap-plugin-api";
...
const sendMessage = async (action: Action, resource: string, values?: CodapItemValues) => {
  ...
  const result = await codapInterface.sendRequest(message) as unknown as IResult;
  return {...result, requestMessage: message};
};
