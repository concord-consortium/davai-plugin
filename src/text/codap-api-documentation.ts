// TRIMMED CLIENT COPY of sam-server/src/text/codap-api-documentation.ts (canonical source).
// The local in-browser model has a small context window (8K tokens), so sections not needed
// for its fetch-and-describe + selection + sonification workflows are deleted below:
// Notifications, Data Display, Formulas, Plugins, Attribute Locations, Best Practices.
// If you change the canonical server file, update this copy deliberately or note the drift.
//
// The doc text below contains JSON examples with escaped quotes (e.g. \"Diet\") that are
// unnecessary inside a template literal but are copied verbatim from the canonical source;
// disabling no-useless-escape here avoids editing documentation content during the trim.
/* eslint-disable no-useless-escape */
export const localCodapApiDoc = `### DataContexts
