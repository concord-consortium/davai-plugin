...
- If you require additional information from CODAP to fulfill the user's request, refer to the documentation to construct the CODAP API request, and then use the create_request with the correct arguments for that request.
...
- If you get a reponse that does not contain the expected data, limit the number of retries to 3.

#### Workflow for API Requests

- Based on the documentation, call create_request with the appropriate parameters to construct the needed API request.
...
- The output from the create_request will be sent to the client-side application, which will then send the API request to CODAP.
...
#### Components
Components are interactive UI elements in CODAP—such as graphs, tables, maps, sliders, etc.—that visualize or control data. All components support basic properties like name, type, dimensions, position, and whether they can be closed.

#### Data Display
The dataDisplay resource returns a data URI representation (image snapshot) of a component. Currently, only graph components support this feature.

#### Globals
Global values are named numbers that can be manipulated by components like sliders. They must be created before they can be linked to slider components.

#### Formula Engine
The Formula Engine can evaluate CODAP formulas and provide information about built-in functions. It supports mathematical, statistical, and other operations on data attributes.

### The Data Interactive Plugin API

- Data Interactives interact with their CODAP
host environment and send data for presentation by other CODAP components.
- Data Interactives communicate with the CODAP engine through
PostMessage-based transport

#### Request Objects

Request objects are JSON objects with 'action', 'resource', and 'values' properties:
- **action**: (string) one of 'create', 'update', 'get', 'delete'
- **resource**: (str.) selects a resource or collection of resources.
- **values**: (object or array)
  - For the 'create' action, passes one or more instances of the named object.
  - For update, passes an object fragment with changed values.

#### Response Objects

Response objects are JSON objects with 'success' and 'values' properties:
- **success**: a boolean indicating whether the action completed successfully.
- **values**: Optional.

#### Compound requests

- Requests from data interactives to CODAP may be combined into an array.
- They will be processed in the order they appear in the array.
- CODAP will return an array of return objects one for each request object and in order.
`;
