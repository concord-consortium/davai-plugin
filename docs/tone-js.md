...
- [website](https://tonejs.github.io/): main page is rendering of repo readme, but seems to missing links to sub pages:
...
The main time calculations are defined in `TickSource#getTicksAtTime` and `TickSource#getSecondsAtTime` which both use `TickSource#_state` and `TickSource#_tickOffset`. However before detailing how it works, its useful to understand the path from `Tone.getTransport().seconds` to this `TickSource#getSecondsAtTime`. The path from `Tone.getTransport().ticks` is basically the same.
...
However this Draw approach isn't great for updating a line or time display. In those cases we want the time display to update as fast as possible. So we want to trigger an update on every animation frame. Using the Draw mechanism for this requires scheduling a loop with a interval that is less than the minium animation frame time. This could result in extra draw events being called, or not enough if the interval is wrong.
...
An approach that seems to work is to track our own transport time and transport state. When the state is "playing" an animation loop updates our transport time with the immediate time `Tone.getTransport().getSecondsAtTime(Tone.immediate())`. When a state event arrives the transport time from event is used `Tone.getTransport().getSecondsAtTime(eventArg0)`. If the animation loop fires when the state is not "playing" then it shouldn't update the transport time.

When a pause or stop event happens our transport time jumps forward by `lookAhead - renderingTime`. Since the number or line stops changing immediately after this jump that seems OK. When a start event happens there might be a couple renders in a row, but they seem to have transport times that are always increasing.

This approach might fail if the lookAhead time is large. In this case the start even will be handled early and the animation frame will start updating our transport time. If the lookAhead time is larger than the gap between a stop and this play, then I think the initial updates from the animation frame will cause our transport time to jump back to the time before the stop. If that becomes a problem then we should store the absolute time of events when they come in and then in the animation frame we make sure the immediate time is after this time before using it to update our transport time. It isn't clear if this would be a real problem though.
