...
//
// This is the essence of the old src/handlers/job-processor.ts with the SQS/RDS
// plumbing removed: no jobs table, no queue, no pg LISTEN. State lives in the in-VM
// MemorySaver keyed by thread_id. The turn loop, tool-repair self-heal, streaming
// accumulation, and response shape are preserved verbatim so the client sees
// identical outputs.
